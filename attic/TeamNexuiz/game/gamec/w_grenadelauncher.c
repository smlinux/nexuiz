...
void(float req) w_glauncher =
{
	if (req == WR_IDLE)
		...
		self.currentammo = floor(self.ammo_rockets / 2);
	else if (req == WR_DROP)
...
void W_Grenade_Explode (void)
{
	...
	RadiusDamage (self, self.owner, cvar("g_balance_grenadelauncher_damage"), cvar("g_balance_grenadelauncher_edgedamage"), cvar("g_balance_grenadelauncher_radius"), world, cvar("g_balance_grenadelauncher_force"), WEP_GRENADE_LAUNCHER);
